...
void* Mem_Alloc(size_t size) {
	...
	Error_SetError("Could not find enough free space to alloc.");
	return NULL;
}

void Mem_Free(void* start_ptr) {
	if (start_ptr == NULL) {
		Error_SetError("Pointer to free is null.");
	}
	...
	Error_SetError("Cannot free a space not reserved.");
}
...
void Mem_AddPtr(char* ptr, size_t size) {
	...
	if (i >= MAX_OBJ) {
		Error_SetError("Max number of objects supported in memory reached.");
		return;
	}
